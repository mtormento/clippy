#include "clippy_mass_storage_view.h"
#include "../clippy_app_i.h"
#include <gui/elements.h>

struct ClippyMassStorage {
    View* view;
};
...
static void clippy_mass_storage_draw_callback(Canvas* canvas, void* _model) {
    ClippyMassStorageModel* model = _model;
...
ClippyMassStorage* clippy_mass_storage_alloc() {
    ClippyMassStorage* mass_storage = malloc(sizeof(ClippyMassStorage));

    mass_storage->view = view_alloc();
    view_allocate_model(mass_storage->view, ViewModelTypeLocking, sizeof(ClippyMassStorageModel));
    with_view_model(
        mass_storage->view,
        ClippyMassStorageModel * model,
        {
    ...
    view_set_draw_callback(mass_storage->view, clippy_mass_storage_draw_callback);
